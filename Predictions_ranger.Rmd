...
## Drop the ModelF variable
This has the same value as ModelCode, but different levels in the two files, Model 3 and Model 4 in the national file, but only Model 4 in the site file.

```{r Remove surplus factor variable}
H.N <- H.N %>% select(-ModelF)
H.S <- H.S %>% select(-ModelF)
```
...
These results are quite different, though both suggest the number of trees is not too important. The model based optimisation strongly suggests 10 to 15 for mtry (the number of variables considered at each split), while for the random search method, the range of mtry considered optimal is from 15 to 25.
...
Interpreting these, suggests that 10, or 11, is the best choice for mtry, and that the choice for num.trees is less critical, but around 2000 seems reasonable.
