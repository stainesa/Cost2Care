...
#####################################################
# Load the working version of the merged HIPE/Study data file
# 
C2C.H <- readRDS('data/C2C.H.Rds')
...
TRAIN <- bind_cols(
  ...
  MedSurg = C2C.H$MedSurg[split_C$train])
TEST <- bind_cols(
  SET = split_C$test,
  Result = 'TEST',
  LosC = C2C.H$LosC[split_C$test],
  MedSurg = C2C.H$MedSurg[split_C$test])
...
Then we look briefly at the importance of the respective variables using a simple ranger model, with no tuning. We could use this to filter variables out. What we see is a small block of variables with importance of zero, and a reasonable range from 0 to 0.15, with another block with, more or less, similar importance of around 0.06 to 0.08.
...
These results are quite different, though both suggest the number of trees is not too important. The model based optimisation strongly suggests 9 for mtry (the number of variables considered at each split), while for the random search method, the range of mtry considered optimal is from 15 to 25.
...
Interpreting these, suggests that a little over 15, perhaps 17 or 18, is the best choice for mtry, and that the choice for num.trees is less critical, but around 2000 seems reasonable.
