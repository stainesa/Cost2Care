...
## Examine medical and surgical admissions in the chart review site, and the main HIPE data
...
#table(H.C$MedSurg)
#table(H.C$AN)
ctable(H.C$MedSurg,H.C$AN, prop = 't')
...
#table(H.S$MedSurg)
#table(H.S$AN)
ctable(H.S$MedSurg,H.S$AN, prop='t')
...
#table(H.A$MedSurg)
#table(H.A$AN)
ctable(H.A$MedSurg,H.A$AN, prop = 't')
...
# 
H.C <- H.C |> # Study site Chart review cases
  group_by(ModelF, HospCode, MedSurg, AN) |>
  summarise(N.C = n())
H.S <- H.S |> # Study site total
  group_by(ModelF, HospCode, MedSurg, AN) |>
  summarise(N.S = n())
H.A <- H.A |> # All Model 3 and Model 4
  group_by(ModelF, HospCode, MedSurg, AN) |>
  summarise(N.A = n())
...
# 
Weights <- H.C |>
  full_join(H.S,by = join_by(ModelF, HospCode, MedSurg, AN))  |>
  mutate(Ratio = N.C/N.S) |>
  mutate(Weight = 1/Ratio) |>
  mutate(Check = N.C * Weight)
...
C2C <- Cost2Care.HIPE |>
  mutate(AN = ifelse(Anaesthesia > 0, 'Yes', 'No'))  |>
  right_join(Weights |> ungroup() |> select(MedSurg, AN, Weight),
