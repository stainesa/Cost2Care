...
# 
ACHI <- as_tibble(
  read_excel('data/H240272_Proc_ACHI_MAP_10thEdn.xlsx', sheet = 'Proc_ACHI_MAP_10thEdn'))
```
...
# There is no particular rational to the ordering of the codes, and it is not meaningful
# the aim here is to generate a set of variable indicating for each person which group of codes they have
#

skimr::skim(ACHI)

HIPE.Site.ACHI <- HIPE.Site <- pivot_longer(HIPE.Site)
```
...
Age at admission does not vary greatly between the main group of hospitals. These are an older group of patients, all aged 65 or over, and admitted for at least three days.
...
A similar pattern can be seen for procedures, although there is some suggestions that practices in individual hospitals may be rather different.
