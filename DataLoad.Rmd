...
HIPE.Site <- as_tibble(read.csv("data/H240272_2022_Site_Study_Subjects.csv"))
```
...
HIPE_FILE <- HIPE_FILE %>%
  unite("ICD_combined", dx.01:dx.30, sep=' ',
        remove = FALSE, na.rm=TRUE) # comorbidity expects a single variable containing all the ICD10 codes to be used as a a strong, hence ICD-combined - This is a *BAD* idea.
...
HIPE_FILE <- HIPE_FILE %>%
  ...
  select(StudyID, ID, Training, ModelF, ModelCode, HospCode,
         ...
         ScoreEl, ScoreCh, HospNo:Outcome,
         hosp.anon:proc.desc.20, MiCh:DepreEl ) # Reorder variables
