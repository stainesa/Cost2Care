...
### Apply key funtion to HIPE data
...
We use this file to add the DRG Complexity codes to the file

```{r AddDRG complexity codes to the files}
...
We prepare a coding file to group ACHI procedures into meaningful group, mostly by ACHI chapter, for Intervention n.e.c. we use the Anatomical sites provided.
...
```{r Merge the group variables back into the main datasets}
#####################################
# HIPE
# 
HIPE <- HIPE |>
...
Next we check and merge the Chart Review data, and the corresponding HIPE.Site data for those cases.
...
Age at admission does not vary greatly between the main group of hospitals. Note that this particular work is confined to an older group of patients, all aged 65 or over, and admitted for at least three days.
...
Note that hadx.01 and hadx30 are both 100% missing. They are in the summary, but as logical variables.
...
* Failure to maintian - i.e. any one or more of the above (F2M)
* Death recorded
...
The HIPE dataset contains data from Model 4, Model 3 and Model 2 hospitals, as a well as the three large maternity hospitals in Dublin, and five specialist hospitals. There is fair evidence that the latter two groups are rather different form the first three. Accordingly we remove them from the saved HIPE file. We also remove the corresponding factor levels from both the national HIPE file, and the chart review site file.


```{r Drop specialist and maternity hospitals and corresponding factor levels}
