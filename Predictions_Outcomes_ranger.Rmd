...
#  tsk_Outcome$col_roles$weights_measure = "Weights"
  tsk_Outcome$select(
    setdiff(tsk_Outcome$feature_names, "Weights"))
...
#  tsk_Outcome$filter(split_Outcome$train) 
    ...
    prediction_test <-
      ...
      IMPORT_f <- importance(tsk_Outcome_f)
        draw_importance(IMPORTANCE = IMPORT_f,
                Outcome_label = "PN (filtered)",
                cut_off = "10th centile")
    lrn_Outcome_f <- trainer(tsk_Outcome_f, split_Outcome)
      lrn_Outcome_f$model
...
roc_outcome = autoplot(prediction_test, type = 'roc') +
    labs(title = 'PN - Pneumonia'); roc_outcome
roc_outcome = autoplot(prediction_test_f, type = 'roc') +
    labs(title = 'PN - Pneumonia f'); roc_outcome
  
  Threshold_optimised =as.numeric(
    SELECT_THRESHOLD(roc_outcome$data))
  prediction_train$set_threshold(Threshold_optimised)
  prediction_test$set_threshold(Threshold_optimised)
...
tune_df <- tune_trainer(tsk_Outcome_f, 50)
  draw_tuners(tune_df, TITLE = "PN - Pneumonia f")
...
autoplot(rr_Outcome_1, type = 'roc') +
  labs(title = "PN - Pneumonia", subtitle = 'Full data set') +
autoplot(rr_Outcome_2, type = 'roc') +
  labs(title = "PN - Pneumonia", subtitle = 'Filtered data set') +
