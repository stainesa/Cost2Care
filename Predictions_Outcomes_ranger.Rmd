...
The commonest adverse event was 'Failure to maintain'. The goal of this work is to predict these.
...
## Split task into test and training sets

```{r function to split task}
...
```{r function to print selected trainer output}
...
It looks if between 5 and 10 is the best value for mtry, so we pick 8; similarly the best range for num.trees is about 1,100 to 1,300, so we pick 1,200. The optimum cutoff for the threshold seems to be about about `r round(Threshold_optimised, 2)`, rather than the default of 0.5.
...
IMPORTANCE <- importance(tsk_Outcome)
  draw_importance(IMPORTANCE = IMPORTANCE,
                ...
                cut_off = 0)
tsk_Outcome
tsk_Outcome_predicted <- 
  trainer(tsk_Outcome, split_Outcome,
                        mtry = 8, num.trees = 1200)
