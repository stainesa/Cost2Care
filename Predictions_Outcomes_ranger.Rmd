...
tsk_Outcome <- tsk_Outcome$filter(split_Outcome$train)
tsk_Outcome_f <- tsk_Outcome_f$filter(split_Outcome$train)
...
rr_Outcome <- resample(
  ...
  rr_Outcome$aggregate(measures)

rr_Outcome_f <- resample(
  task = tsk_Outcome_f,
  ...
  rr_Outcome_f$aggregate(measures)

autoplot(rr_Outcome, type = 'roc') +
  labs(title = "PN - Pneumonia", subtitle = 'Full data set') +
autoplot(rr_Outcome_f, type = 'roc') +
...
# Predictions
#
Lfull     <- rr_Outcome$learners[[1]]
Lfiltered <- rr_Outcome$learners[[1]]
