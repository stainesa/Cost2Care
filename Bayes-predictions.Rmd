...
# 
saveRDS(df.train.PN, file = "data/df.train.PN")
saveRDS(df.train.PNk, file = "data/df.train.PNk")

saveRDS(df.train.UTI, file = "data/df.train.UTI")
saveRDS(df.train.UTIk, file = "data/df.train.UTIk")

saveRDS(df.train.PU, file = "data/df.train.PU")
saveRDS(df.train.PUk, file = "data/df.train.PUk")

saveRDS(df.train.DL, file = "data/df.train.DL")
saveRDS(df.train.DLk, file = "data/df.train.DLk")

saveRDS(df.train.F2M, file = "data/df.train.F2M")
saveRDS(df.train.F2Mk, file = "data/df.train.F2Mk")

saveRDS(df.test.PN, file = "data/df.test.PN")
saveRDS(df.test.PNk, file = "data/df.test.PNk")

saveRDS(df.test.UTI, file = "data/df.test.UTI")
saveRDS(df.test.UTIk, file = "data/df.test.UTIk")

saveRDS(df.test.PU, file = "data/df.test.PU")
saveRDS(df.test.PUk, file = "data/df.test.PUk")

saveRDS(df.test.DL, file = "data/df.test.DL")
saveRDS(df.test.DLk, file = "data/df.test.DLk")

saveRDS(df.test.F2M, file = "data/df.test.F2M")
saveRDS(df.test.F2Mk, file = "data/df.test.F2Mk")
...
saveRDS(df.HIPE.PN, file = "data/df.HIPE.PN")
saveRDS(df.HIPE.PNk, file = "data/df.HIPE.PNk")

saveRDS(df.HIPE.UTI, file = "data/df.HIPE.UTI")
saveRDS(df.HIPE.UTIk, file = "data/df.HIPE.UTIk")

saveRDS(df.HIPE.PU, file = "data/df.HIPE.PU")
saveRDS(df.HIPE.PUk, file = "data/df.HIPE.PUk")

saveRDS(df.HIPE.DL, file = "data/df.HIPE.DL")
saveRDS(df.HIPE.DLk, file = "data/df.HIPE.DLk")

saveRDS(df.HIPE.F2M, file = "data/df.HIPE.F2M")
saveRDS(df.HIPE.F2Mk, file = "data/df.HIPE.F2Mk")
```
...
make_Predictions <- function (name, probs = c(0.10, 0.5, 0.90)) {
  fit = paste0('fit.',name)
  HIPE = paste0('df.HIPE.',name)

fit   <- readRDS(paste0('data/fit.',name))
HIPE  <- readRDS(paste0('data/df.HIPE.',name))
...
We see a striking difference between the Model 3 and the Model 4 hospitals, with higher predicted adverse event rates in the Model 4 hospitals. Given the casemix they see, this is not surprising. The chart review was done n one Model 4 hospital, and accordingly has higher observed adverse event rates than the overall rates.
