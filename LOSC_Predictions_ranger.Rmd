...
```{r Load data files}
#####################################################
# Load the working version of the merged HIPE Study site file
# 
CTC.H <- readRDS('data/CTC.H.Rds')


#####################################################
# Load the working version of the merged HIPE Study site file
# 
H.S <- readRDS('data/H.S.Rds')
H.S <- H.S %>%
  select(-c(ModelF, ModelCode, HospCode))

#####################################################
# Load the working version of the total HIPE data file
# 
H.N <- readRDS('data/H.N.Rds')
H.N <- H.N %>%
  select(-c(ModelF, ModelCode, HospCode))
...
We actually model the scaled length of stay which has a mean of zero (but a  median of -0.3), and a standard deviation of 1, while the actual length of stay has a mean of 15.5, (but a median of 9), and a standard deviation of 20.

## lmer based predictions of Length of Stay (centred and scaled)

```{r Simple lmer model}
ModelLOS <- lmer(data=CTC.H,
             LosC ~
               AgeC + as_factor(sex) + Elixhauser +
...
PRED <- predict(ModelLOS, newdata = CTC.H)

C <- bind_cols(LosC = CTC.H$LosC, PRED = PRED, sex = CTC.H$sex)
...
RANGER1 <- ranger(
    data = CTC.H %>% select(-Training),
    formula = LosC ~.,
        num.trees = 1000,
    mtry = 11,
    importance = "permutation",
    case.weights = CTC.H$Training,
    holdout = TRUE
)
...
#  
RANGER1.train.predict <- predict(RANGER1, data = CTC.H %>% filter(Training))
  summary(RANGER1.train.predict$predictions)
    PRED.train <- bind_cols(LosC = CTC.H$LosC[CTC.H$Training],
                            Prediction = RANGER1.train.predict$predictions,
                            Train = TRUE)

RANGER1.test.predict <- predict(RANGER1, data = CTC.H %>% filter(!Training))
  summary(RANGER1.test.predict$predictions)
    PRED.test <- bind_cols(LosC = CTC.H$LosC[!CTC.H$Training],
...
First we set up a regression task. We split the study site HIPE (H.S)data into a two-thirds training set, and a one third testing set, stratified by MedSurg.
...
#
tsk_C <- as_task_regr(H.S, target = "LosC", id = 'C')
  tsk_C$col_roles$stratum = c("MedSurg")
  tsk_C
...
TRAIN <- bind_cols(
  SET = split_C$train,
  Result = 'TRAIN',
  LosC = CTC.H$LosC[split_C$train],
  MedSurg = CTC.H$MedSurg[split_C$train])
TEST <- bind_cols(
  SET = split_C$test,
  Result = 'TEST',
  LosC = CTC.H$LosC[split_C$test],
  MedSurg = CTC.H$MedSurg[split_C$test])
SPLIT <- bind_rows(TRAIN,TEST)

ggplot(data = SPLIT,
       aes(y = LosC, group = Result,
           x = Result)) +
  geom_boxplot() +
...
Then we look briefly at the importance of the respective variables using a simple ranger model, with no tuning. We could use this to filter variables out. What we see is a smooth scree ranging from 0 to 0.18, a small block of variables with importance of zero, and , with another block with, more or less, similar importance of around 0.06 to 0.08.
...
First set up a basic model to tune, in this case a random forest regression. We're tuning on mtry, which is the number of variables to possibly split at in each node, with a default value of 8 where we're looking at 5 to 25; and the number of trees to look at, which is to an extent, arbitrary but we're looking from 500 to 3,000 per run.
...
We set up two instances, which are identical apart from the tuners used. One is 2,000 runs of a random search model, and one 750 runs of a model based optimisation model. They cover the parameter space in different ways. Both are very slow to run.
...
ggplot(data = ParameterSearchResults_mbo,
  ...
  labs(title = ("Model Based Optimization"))

ggplot(data = ParameterSearchResults_random_search,
       aes(y = num.trees, x = mtry)) +
  geom_jitter(aes(size = regr.mse, colour= regr.mse)) +
  labs(title = ("Random Search Based Optimization"))
...
g1 <- ggplot(data = ParameterSearchResults_mbo,
  ...
  geom_jitter(aes(size = regr.mse, colour = as.integer(mtry))) + #mtry is an integer, but of type numeric, which triggers an error
...
These results are similar, though both suggest the number of trees, once over 500 or so, is not too important. Both methods suggest 5 for mtry (the number of variables considered at each split).
...
TRAIN <- bind_cols(
  ...
  Result = 'TRAIN',
  LosC = H.S.trim$LosC[split_C.trim$train])
TEST <- bind_cols(
  SET = split_C.trim$test,
  Result = 'TEST',
  LosC = H.S.trim$LosC[split_C.trim$test])
SPLIT <- bind_rows(TRAIN,TEST)
...
Then we look briefly at the importance of the respective variables using a simple ranger model, with no tuning. The range and pattern of importances are similar to the untrimmed dataset.
...
ggplot(data = ParameterSearchResults_mbo.trim,
       aes(y = num.trees, x = mtry)) +
  geom_jitter(aes(size = regr.mse, colour= regr.mse)) +
  labs(title = ("Model Based Optimization trimmed"))


ggplot(data = ParameterSearchResults_random_search.trim,
       aes(y = num.trees, x = mtry)) +
  geom_jitter(aes(size = regr.mse, colour= regr.mse)) +
  labs(title = ("Random Search Based Optimization trimmed"))
...
g1 <- ggplot(data = ParameterSearchResults_mbo.trim,
       aes(x = num.trees, y = regr.mse)) +
  geom_jitter(aes(size = regr.mse, colour = as.integer(mtry))) +
  labs(title = "Model Based Optimisation") +
  theme_minimal()
...
Interpreting these, suggests that 8, is the best choice for mtry, and that the choice for num.trees is less critical, but around 2000 seems reasonable.

# Final model
This model uses the tuned values derived above for the trimmed model.
...
These are predictions of the test data, which was not used to train the model.
...
OUTCOME <- as_tibble(cbind(Truth = prediction$truth, Predicted = prediction$response))
  OUTCOME$Difference = OUTCOME$Truth - OUTCOME$Predicted

ggplot(data = OUTCOME, aes(y = Truth, x = Predicted)) +
  geom_jitter() +
  geom_smooth() +
  labs(title = "MBO predictions of scaled LOS for trimmed data")

ggplot(data = OUTCOME, aes(x = Truth, y = Difference)) +
  geom_jitter() +
  geom_smooth() +
  labs(title = "MBO residudals of scaled LOS for trimmed data")
```

This suggests an acceptable, but far from perfect prediction, with a tendency to underestimate the LOS for shorter stays.

## Predict LosC for trimmed national HIPE data

```{r}
H.N.trim <- H.N %>% filter(LosC <= TRIM_LosC)

HIPE.prediction <- lrn_R_mbo$predict_newdata(task = tsk_C.trim, newdata = H.N.trim)
...
ggplot(data = sample_n(OUTCOME, 2000), aes(x = Truth, y = Predicted)) +
  geom_jitter(alpha = 0.25) +
  geom_smooth() +
  labs(title = "MBO predictions of scaled LOS for trimmed data")

ggplot(data = sample_n(OUTCOME, 2000), aes(x = Truth, y = Difference)) +
  geom_jitter(alpha = 0.25) +
  geom_smooth() +
  labs(title = "MBO residudals of scaled LOS for trimmed data")
```
