...
# Dental.Services has only 3, which is really too small to be any use, so we lose it too.
```
...
```{r}
table(df.train.PN$Source)
table(df.train.PNk$Source)
table(df.train.UTI$Source)
table(df.train.UTIk$Source)
table(df.train.PU$Source)
table(df.train.PUk$Source)
table(df.train.DL$Source)
table(df.train.DLk$Source)
table(df.train.F2M$Source)
table(df.train.F2Mk$Source)
...
We have 4000 estimates for each of 10 models of the predicted value of the probability of the outcome (Pn, UTI, ... F2M), derived from the models we have fitted. For the Chart review data we also have the ground truth, in a variable 'Result'. Clearly we won't have this for the HIPE data.

We want three things (for each model)

* A plot of the distribution of the predictions
