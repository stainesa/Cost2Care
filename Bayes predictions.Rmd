...
make_dwPlot_fit(fit.PN)
make_dwPlot_fit(fit.PNk)

make_dwPlot_fit(fit.UTI)
make_dwPlot_fit(fit.UTIk)

make_dwPlot_fit(fit.PU)
make_dwPlot_fit(fit.PUk)

make_dwPlot_fit(fit.DL)
make_dwPlot_fit(fit.DLk)

make_dwPlot_fit(fit.F2M)
make_dwPlot_fit(fit.F2Mk)
...
This tells us a number of things. The events under consideration are relatively uncommon, with prevalences mostly under 10$% in the chart review. The predictions on the training data are pretty good, which is to be expected. The predictions on the test data are also pretty good, which suggests that the modelling has worked reasonably well, and the it is possible to estimate the prevalence of these hospital acquired complications from features derived directly from routine data.
...
We draw 2000 predictions of epred (which is the probability of having an adverse event) for each of the 10 models.
...
rm(probs,
   str_PN, str_PNk, str_UTI, str_UTIk,
   str_PU, str_PUk, str_DL, str_DLk,
   str_F2M, str_F2Mk)
```
...
gc(reset = TRUE, full = TRUE)
    ...
    Vnames = paste0('V', 1:4000)
        Vsample <- sample(Vnames, size=1000, replace = FALSE)
    rm(Vnames)
...
#  HIPE.Predicted <-
 XX <-   readRDS(file = 'data/Predictions_PN.Rds')# |>
  select(c(rowid,Q10:Outcome, Vsample)) |>
...
```{r Graph HIPE prediction}
##############################################
# Reload and process the original data
CCH <- readRDS('data/CCH.Rds')
CCH <- CCH |>
  mutate(across(where(is.character), ~as_factor(.)))  |>
  rowid_to_column(var = 'rowid')
...
```{r Summarise HIPE predictions}
#########################################
# reload HIPE predictions data
#
make_summary_row <- function(name) {
    filename = paste0('data/Predictions_', name, '.Rds')
    ROW <-  readRDS(filename) |>
        select(Outcome, Q50) |>
        group_by(Outcome) |>
...
rm(HIPE_PN, HIPE_PNk, HIPE_UTI, HIPE_UTIk,
   HIPE_PU, HIPE_PUk, HIPE_DL, HIPE_DLk,
    HIPE_F2M, HIPE_F2Mk)
```

```{r Plot of all hospitals}
ggplot(HIPE.Predicted,
       aes(x = Q50, colour = Outcome)) +
  geom_density() +
  ...
  theme_minimal() +
  guides(colour = 'none') + 
  labs(title = 'Predictions on HIPE data',
       x = 'Prediction') +
    coord_cartesian(xlim = c(0,0.3)) +
  facet_wrap( ~Outcome )
```
...
```{r HIPE breakdown}
#######################################
# Link HIPE.Predicted to relevant variables
# from HIPE

HIPE.Predicted <- HIPE.Predicted |>
  full_join(HIPE |>  rowid_to_column('rowid') |> select(rowid, HospCode, ModelF), by = join_by(rowid))


#########################################
# Summary of test predictions
Summary.HIPE.Model <- HIPE.Predicted |>
  select(ModelF, Outcome, Q50) |>
  group_by(ModelF, Outcome) |>
  summarise(across(Q50, list(mean = mean, median = median, sd = stats::sd)))  |> # 20 values
  ungroup() |>
  select(Outcome, ModelF, Q50_mean, Q50_median, Q50_sd ) |>
  arrange(Outcome, ModelF)

kable(Summary.HIPE.Model, caption = 'Summary of modelled predictions for full HIPE')
```

This shows us that the predicted complication rates for Model 3 hospitals, as a group, are lower than those for Model 4 hospitals as a group. The next two graphs explore this further.

```{r}
ggplot(HIPE.Predicted,
       aes(x = Q50, colour = Outcome)) +
  geom_density(data = HIPE.Predicted |> filter(ModelF == 'Model 3'), linewidth = 2) +
  geom_vline(data = Summary.HIPE.Model |>
  ...
  guides(colour = 'none') + 
  labs(title = 'Predictions on HIPE data',
       subtitle = 'Model 3 hospitals only',
       x = 'Prediction') +
    coord_cartesian(xlim = c(0,0.3)) +
  facet_wrap( ~Outcome )
...
ggplot(HIPE.Predicted,
  ...
  guides(colour = 'none') + 
  labs(title = 'Predictions on HIPE data',
       subtitle = 'Model 4 hospitals only',
       x = 'Prediction') +
    coord_cartesian(xlim = c(0,0.3)) +
  facet_wrap( ~Outcome )
