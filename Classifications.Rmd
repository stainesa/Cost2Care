...
```{r Run all 5 models for the two learners}
...
1) The importance reduced datasets fit better
2) a good value for mtry is around 3
...
```{r PNK_predict}
PNk_trained <- lrn_T$train(tsk_PNk, row_ids = split_PN$train)
  PNk_pred <- PNk_trained$predict(tsk_PNk, row_ids = split_PN$test)
...
```{r UTIk_predict}
UTIk_trained <- lrn_T$train(tsk_UTIk, row_ids = split_PN$train)
  UTIk_pred <- UTIk_trained$predict(tsk_UTIk, row_ids = split_PN$test)
...
```{r PUk_predict}
PUk_trained <- lrn_T$train(tsk_PUk, row_ids = split_PN$train)
  PUk_pred <- PUk_trained$predict(tsk_PUk, row_ids = split_PN$test)
...
```{r DLk_predict}
DLk_trained <- lrn_T$train(tsk_DLk, row_ids = split_PN$train)
  DLk_pred <- DLk_trained$predict(tsk_DLk, row_ids = split_PN$test)
...
```{r F2Mk_predict}
F2Mk_trained <- lrn_T$train(tsk_F2Mk, row_ids = split_PN$train)
  F2Mk_pred <- F2Mk_trained$predict(tsk_F2Mk, row_ids = split_PN$test)
...
Test_predictions <- tibble(dt_PNk, dt_UTIk, dt_PUk, dt_DLk, dt_F2Mk, .name_repair = 'universal')
names(Test_predictions) <- c('PNk','UTIk','PUk','DLk','F2Mk')
