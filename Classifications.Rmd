...
tsk_PN <- as_task_classif(CCH %>%
  ...
  split_PN = mlr3::partition(tsk_PN)
    ...
    table(CCH$PN[split_PN$train])
    table(CCH$PN[split_PN$test])
...
tsk_PNk <- as_task_classif(CCH %>%
  ...
  split_PNk = mlr3::partition(tsk_PNk)
    ...
    table(CCH$PN[split_PNk$train])
    table(CCH$PN[split_PNk$test])
...
tsk_UTI <- as_task_classif(CCH %>%
  ...
  split_UTI = mlr3::partition(tsk_UTI)
    ...
    table(CCH$UTI[split_UTI$train])
    table(CCH$UTI[split_UTI$test])
...
tsk_UTIk <- as_task_classif(CCH %>%
  ...
  split_UTIk = mlr3::partition(tsk_UTIk)
    ...
    table(CCH$UTI[split_UTIk$train])
    table(CCH$UTI[split_UTIk$test])
...
tsk_PU <- as_task_classif(CCH %>%
  ...
  split_PU = mlr3::partition(tsk_PU)
    ...
    table(CCH$PU[split_PU$train])
    table(CCH$PU[split_PU$test])
...
tsk_PUk <- as_task_classif(CCH %>%
  ...
  split_PUk = mlr3::partition(tsk_PUk)
    ...
    table(CCH$PU[split_PUk$train])
    table(CCH$PU[split_PUk$test])
...
tsk_DL <- as_task_classif(CCH %>%
  ...
  split_DL = mlr3::partition(tsk_DL)
    ...
    table(CCH$DL[split_DL$train])
    table(CCH$DL[split_DL$test])
...
tsk_DLk <- as_task_classif(CCH %>%
  ...
  split_DLk = mlr3::partition(tsk_DLk)
    ...
    table(CCH$DL[split_DLk$train])
    table(CCH$DL[split_DLk$test])
...
tsk_F2M <- as_task_classif(CCH %>%
  ...
  split_F2M = mlr3::partition(tsk_F2M)
    ...
    table(CCH$F2M[split_F2M$train])
    table(CCH$F2M[split_F2M$test])
...
tsk_F2Mk <- as_task_classif(CCH %>%
  ...
  split_F2Mk = mlr3::partition(tsk_F2Mk)
    ...
    table(CCH$F2M[split_F2Mk$train])
    table(CCH$F2M[split_F2Mk$test])
...
Test_predictions.H_nested <- Test_predictions.H |> group_by(Outcome) |> nest()
...
This decision, in turn, depends on the choice of a threshold, a cutoff of probability above which the record is classified as positive and below which it is classified as negative. The default cutoff is 0.5, but this has been adjusted to the cutoff from the ROC curve, which minimise the difference between the sensitivity and the specificity (1. Jiménez-Valverde A, Lobo JM. Threshold criteria for conversion of probability of species presence to either–or presence–absence. Acta Oecologica. 2007 May 1;31(3):361–9). These thresholds, derived as part of the analysis above, are used in the class prediction here.
...
TP.prob <- TP |>
  ...
  summarise(Prob = sum(prob.Yes)/330)
...
group_by(Model, Outcome_name) |>
  summarise(Prob = mean(Yes),
            Prob.lower = Prob - 2*sd(Yes),
            Prob.upper = Prob + 2*sd(Yes))
...
the next step is to examine these at hospital level. There are 17 Model 3 hospitals and 9 Model 4 hospitals in these data. Model 3 hospitals primarily provide routine acute general hospital services. Model 4 hospitals both serve the function of Model 3 hospitals, and, usually several, regional or national speciality services. The number of discharges included from model 3 hospitals ranges from 1779 to 4965, and those in Model 4 centres from 5906 to 8868

```{r Hospital discharge table}
ctable(HIPE$HospCode, HIPE$ModelF,  weights = rep(1,length(HIPE$ModelF)), prop = 'n', totals = FALSE)
...
```{Plot of probabilities by hospital}
ggplot(TP.H,
...
A more informative graph, when it comes to looking at differences between hospitals, is a graph of the differences from the overall mean. A graph against the unweighted hospital level mean is very similar.

```{Plot of differences by hospital}
ggplot(TP.H,
