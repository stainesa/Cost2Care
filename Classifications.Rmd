...
```{r Check data}
HIPE_names <- read_csv('data/HIPE_names.csv')
HIPE.Trimmed_names <- read_csv('data/HIPE.Trimmed_names.csv')
tsk_PN_names <- tsk_PN$feature_types

HIPE_names <- HIPE_names |>
  full_join(tsk_PN_names, by = join_by(Name == id))
HIPE_names |> filter(Type == '<fct>')
HIPE.Trimmed_names <- HIPE.Trimmed_names |>
  full_join(tsk_PN_names, by = join_by(Name == id))
HIPE.Trimmed_names |> filter(Type == '<fct>')

table(HIPE$Complexity)
table(HIPE.Trimmed$Complexity)
table(CCH$Complexity)
...
Test_predictions.H_nested <- Test_predictions |> group_by(Outcome) |> nest()
...
Chart_review <- Truth |>
  full_join(Prediction,
            ...
            suffix = c('.T','.P')) |>
  arrange(Outcome, Result, n.T, n.P, prop.T, prop.P, pct.T, pct.P)
