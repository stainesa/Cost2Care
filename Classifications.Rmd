...
RESULT_test <- TP.prob |>
  full_join(CR, by = join_by(Model, Outcome)) |>
  mutate(Ratio = 100 * round(Prob/prop,4)) |>
  mutate(Prob = 100 * round(Prob,4)) |>
  select(Model, Outcome, Prob, pct, Ratio, everything())

ggplot(TP, aes(fill = Outcome)) +
  ...
  geom_vline(data = TP.prob, #  PREDICTED from chart review test
          aes(xintercept = Prob, colour = Outcome),
          linewidth = 1) +
  facet_wrap(~ Outcome + Model, scales = 'free_y') +
  theme_minimal() +
  ...
  labs(title = "Predicted probabilities for Chart review TEST set",
       subtitle  = 'Vertical lines are observed results, and predicited results',
