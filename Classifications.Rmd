...
```{EVALUATE at T = 0.5, eval=FALSE}

rr_C.f = resample(tsk_PN, lrn_featureless, cv10, store_models = TRUE)
rr_C.rng = resample(tsk_PN, lrn_C, cv10, store_models = TRUE)
...
prediction_C.f = lrn_featureless$predict(tsk_PN, split_PN$test)
prediction_C.f$score(measures)
prediction_C.f$confusion
autoplot(prediction_C.f, type = 'roc')
autoplot(rr_C.f, type = 'prc')

prediction_C.rng = lrn_C$predict(tsk_PN, row_ids = split_PN$test)
...
pred_PNk$set_threshold(0.3)
  pred_PNk$confusion # Much better
autoplot(pred_PNk) + labs(title = 'PNk Threshold 0.3')
roc_PNk
prc_PNk <- autoplot(rr_PNk.rng, type='prc')
TEST <- prc_PNk$data |>
  mutate(FMeasure = (4*x*y)/(x+y))
#TEST <- roc_PNk$data |>
#  mutate(Gmeans = 1- sqrt(y * (1-x)))

summary(TEST)
ggplot(TEST, aes(x = x, y = FMeasure)) +geom_line()
```
