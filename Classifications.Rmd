...
prediction = lrn_featureless$predict(tsk_C, split_C$test)
prediction
prediction$score(measures)
prediction$confusion
autoplot(prediction, type = 'roc')

lrn_C$train(tsk_C, split_C$train)
prediction = lrn_C$predict(tsk_C, split_C$test)
prediction
prediction$score(measures)
prediction$confusion
autoplot(prediction, type = 'roc')
...
instance_C = ti(
  ...
  resampling = cv10,
  measures = msrs(c('classif.auc','classif.bbrier','classif.logloss','classif.acc')),
  terminator = trm("evals", n_evals=50), # For tuner "random_search"
...
Best shot at threshold is about 0.65

# Fit at optimised threshold

```{r}
prediction$set_threshold(0.65)
prediction$score(measures)
prediction$confusion
glimpse(as.data.table(prediction))
```

# Prediction on HIPE data

```{r PREDICT HIPE}
H.Predicted = lrn_C$predict_newdata(HIPE.Trimmed, tsk_C)
H.Predicted$set_threshold(0.65)
H.Predicted$score(measures)
H.Predicted$confusion
