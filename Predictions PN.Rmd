...
C2C.H <- Cost2Care.HIPE %>%
        filter(!is.na(PN)) %>%
    select(PN, Training:proc.desc.20,) %>%
...
ctable(PN <- as_factor(Cost2Care.HIPE$PN),
        PN_Associated <- as_factor(Cost2Care.HIPE$PN_Associated),
  prop = "r")


ctable(pneumonia <- as_factor(Cost2Care.HIPE$pneumonia),
       PN <- as_factor(Cost2Care.HIPE$PN),
  prop = "r")

ctable(H.PN <- as_factor(Cost2Care.HIPE$H.PN),
       PN <- as_factor(Cost2Care.HIPE$PN),
  prop = "r")
...
There seems to be an excess of codes, so lets go with Pneumonia (i.e. PN) for a minute.

## lmer based predictions of PN

```{r Sample glmer model, eval = FALSE}
ModelPN <- glmer(data=C2C.H,
             as_factor(PN) ~ AgeC + as_factor(sex) + ScoreEl +
...
PRED <- predict(ModelPN, newdata = C2C.H)
...
RANGER1 <- ranger(
    data = C2C.H %>%
        select(-Training),
    formula = as_factor(PN) ~.,
        num.trees = 1000,
    mtry = 11,
    probability = TRUE,
    importance = "permutation",
    case.weights = C2C.H$Training[!is.na(C2C.H$PN)],
    holdout = TRUE
)
...
```{r Define and draw a classification task and a split}
#
# Define a classification task for PN
# 
#
tsk_PN <- as_task_classif(C2C.H, target = "PN", id = 'PN')
  tsk_PN
...
TRAIN <- bind_cols(
  SET = split_C$train,
  Result = 'TRAIN',
  PN = C2C.H$PN[split_C$train])
TEST <- bind_cols(
  SET = split_C$test,
  Result = 'TEST',
  PN = C2C.H$PN[split_C$test])
...
flt_Imp = flt("importance", learner = lrn_Filter)
  ...
  Importance_Filtered <-
    ...
    arrange(score, feature)
     ...
     Importance_Filtered %>% filter(score > 0.01) %>% select (-score)
     Importance_Filtered %>% filter(score <= 0.01 & score > 0.008) %>% select (-score)
     Importance_Filtered %>% filter(score <= 0.008 & score > 0.004) %>% select (-score)
     Importance_Filtered %>% filter(score <= 0.004 & score > 0) %>% select (-score)
     Importance_Filtered %>% filter(score == 0) %>% select (-score)

```

Roughly, all the hadx scores are non-contributory. The main contributors are the diagnoses, the length of stay,  and the proc scores, which need to be recoded.


## Run a ranger task, and see what we get
