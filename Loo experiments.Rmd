...
df.train.PNk <- readRDS('data/df.train.PNk')
...
Split CCH into expanded (one row per integer weight) training and test sets per task. Each person is in either the test or training dataset - no-one is in both.
...
fit.hs.PU <- fit.task(df.exp.train.PU, Adapt_Delta = 0.999)
fit.hs.PUk <- fit.task(df.exp.train.PUk, Adapt_Delta = 0.999)

fit.hs.DL <- fit.task(df.exp.train.DL, Adapt_Delta = 0.999)
fit.hs.DLk <- fit.task(df.exp.train.DLk, Adapt_Delta = 0.999)

fit.hs.F2M <- fit.task(df.exp.train.F2M, Adapt_Delta = 0.999)
fit.hs.F2Mk <- fit.task(df.exp.train.F2Mk, Adapt_Delta = 0.999)
...
launch_shinystan(fit.hs.PN, ppd = TRUE) # Allows me to check fit etc.
launch_shinystan(fit.hs.PNk, ppd = TRUE) # Allows me to check fit etc.

launch_shinystan(fit.hs.UTI, ppd = TRUE) # Allows me to check fit etc.
launch_shinystan(fit.hs.UTIk, ppd = TRUE) # Allows me to check fit etc.

launch_shinystan(fit.hs.PU, ppd = TRUE) # Allows me to check fit etc.
launch_shinystan(fit.hs.PUk, ppd = TRUE) # Allows me to check fit etc.

launch_shinystan(fit.hs.DL, ppd = TRUE) # Allows me to check fit etc.
launch_shinystan(fit.hs.DLk, ppd = TRUE) # Allows me to check fit etc.

launch_shinystan(fit.hs.F2M, ppd = TRUE) # Allows me to check fit etc.
launch_shinystan(fit.hs.F2Mk, ppd = TRUE) # Allows me to check fit etc.
