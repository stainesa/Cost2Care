...
The ranger predictions are quite good too, though still not great at high lengths of stay. There are a large number of important variables.
...
# Copy instance to do two tunings
instance_mbo <- instance
instance_random_search <- instance
...
ParameterSearchResults_mbo <- as.data.table(instance_mbo$archive)  %>%
  select(mtry, num.trees, regr.mse)
ParameterSearchResults_random_search <- as.data.table(instance_random_search$archive) %>%
  select(mtry, num.trees, regr.mse)

ggplot(data = ParameterSearchResults_mbo, aes(y = num.trees, x = mtry)) +
  geom_jitter(aes(size = regr.mse, colour= regr.mse))
autoplot(instance_mbo, type = "surface") + scale_color_brewer(palette = "Set1")

ggplot(data = ParameterSearchResults_random_search, aes(y = num.trees, x = mtry)) +
  geom_jitter(aes(size = regr.mse, colour= regr.mse))
autoplot(instance_random_search, type = "surface") + scale_color_brewer(palette = "Set1")
