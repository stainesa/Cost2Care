...
First we set up a regression task. A plot of this is a plot of the outcome variable, in this case centred and scaled length of stay. Note that this is very very skew, with a long tail of outliers, and a designed minimum of 3 days.

```{r Define a regression task}
#
...
#
# Strongly suggests 11 or so for mtry, but that the number of trees matters much less
# 
#
```
...
For this reason we repeat the exercise with a trimmed data set, excluding extreme outliers. The rational for this is that while these very long lengths of stay matter, they are probably driven by very specific individual factors, which will not be captured by any retrospective routine data system.
...
# Define a regression task for LosC
#
tsk_C_trimmed <- as_task_regr(C2C.H %>% filter(LosC <= 2.5),
                              target = "LosC", id = 'C_trimmed')
  tsk_C_trimmed
